...
async def run_scrape_job(urls: list[str]):
    ...
    print(f"\nData quality:")
    print(f"- Products with ASINs: {df.get('asin', pd.Series(dtype=object)).notna().sum()}")
    print(f"- Products with URLs: {df.get('product_url', pd.Series(dtype=object)).notna().sum()}")
    print(f"- Products with titles: {df.get('title', pd.Series(dtype=object)).notna().sum()}")
    print(f"- Products with prices: {df.get('price', pd.Series(dtype=float)).notna().sum()}")
    print(f"- Products with ratings: {df.get('rating', pd.Series(dtype=float)).notna().sum()}")
    print(f"- Products with movers_rank: {df.get('movers_rank', pd.Series(dtype='Int64')).notna().sum()}")
    print(f"- Products with sales_rank_now: {df.get('sales_rank_now', pd.Series(dtype='Int64')).notna().sum()}")
    print(f"- Products with sales_rank_before: {df.get('sales_rank_before', pd.Series(dtype='Int64')).notna().sum()}")
