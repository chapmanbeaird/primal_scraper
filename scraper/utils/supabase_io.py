...
def _fetch_current_rows(supabase: Client, category_name: str) -> pd.DataFrame:
    """Fetch all is_current=true rows for a category."""
    response = supabase.table("category_products") \
        .select("asin, rank, price, reviews, rating, start_date") \
        .eq("category_name", category_name) \
        .eq("is_current", True) \
        .execute()

    if not response.data:
        return pd.DataFrame(columns=["asin", "rank", "price", "reviews", "rating", "start_date"])
...
def add_to_supabase(df: pd.DataFrame):
    """
    SCD Type II implementation for category_products.

    Only creates new versions when tracked attributes (rank, price, reviews, rating) actually change.

    Test cases:
    - Example 1 (no change): rank=51, price=42.48 -> same -> no update
    - Example 2 (rank change): rank=51 -> rank=49 -> close old, insert new
    - Example 3 (drop-off): rank=49 -> missing -> close old, insert rank=NULL
    - Example 4 (re-entry): rank=NULL -> rank=36 -> close drop, insert ranked
    """
    SUPABASE_URL = os.getenv("SUPABASE_URL")
    SUPABASE_KEY = os.getenv("SUPABASE_KEY")
    supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

    TRACKED_COLS = ["rank", "price", "reviews", "rating"]
    BATCH_SIZE = 800
    ...
    if "category_name" not in df.columns:
        raise ValueError("DataFrame must include 'category_name' column.")
    ...
    # Build df_categories with duplicate rank logging
    df_cat_raw = (
        df[["asin", "category_name", "price", "rank", "reviews", "rating"]]
        .sort_values(["category_name", "rank", "asin"])
        .drop_duplicates(subset=["category_name", "asin"], keep="first")
    )

    df_categories = df_cat_raw
    ...
    categories_scraped = df_categories["category_name"].unique().tolist()

    for category_name in categories_scraped:
        cat_data = df_categories[df_categories["category_name"] == category_name]

        # Build today_map: Dict[asin] -> {rank, price, reviews, rating}
        ...
        for asin in to_check_dropoffs:
            ...
            # Only create drop-off if current rank IS NOT NULL (not already dropped)
            if db_rank is not None and not pd.isna(db_rank):
                if category_name not in rows_to_close_by_category:
                    rows_to_close_by_category[category_name] = []
                rows_to_close_by_category[category_name].append((asin, db_row.get("start_date")))

                # Insert drop-off row with rank=NULL
                rows_to_insert.append({
                    "asin": asin,
                    "category_name": category_name,
                    "rank": None,
        ...
        dropoff_count = len([a for a in to_check_dropoffs if current_map[a]["rank"] is not None and not pd.isna(current_map[a]["rank"])])
    ...
    if rows_to_insert:
        ...
        print(f"✅ category_products insert complete ({total} rows)")
    else:
        print("✅ No changes detected - no rows to insert")

    # Phase E: Update category_last_updated
